...
-- Query 2
-- расчет сумм выданных овердрафтов по месяцам в соотношении с просроченными с нарастающим итогом

-- извлечение месяца и расчет суммы просроченных и выданных по месяцам
WITH
  granted AS (
  SELECT
    EXTRACT(month
...
--замена порядкового числа месяца на текст, расчет сумм выданных и просроченных нарастающим итогом
...
-- Query 5
  --расчет суммы просроченных овердрафтов в зависимости от срока возникновения просрочки
SELECT
...
-- Query 6
  --расчет среднего количества дней оплаты просрочки, расчет частоты оплаты по дням после наступления просрочки 
